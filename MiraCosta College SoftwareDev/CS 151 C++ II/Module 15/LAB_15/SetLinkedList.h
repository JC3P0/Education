...
public:
    ...
    Set<T> minus(Set<T> other_set) const;
...
template<typename T>
Set<T> Set<T>::minus(Set<T> other_set) const {
    Set<T> return_set; //return set
    SetNode<T> *ptr = head;
    while(ptr != nullptr) {
        if(!other_set.contains(ptr -> data)){
            return_set.insert(ptr -> data); //add elements not in other_set
        }
        ptr = ptr -> next; //cycle through nodes while looping
    }
    return return_set;
}
...
bool Set<T>::equals(Set<T> other_set) {
    ...
    while(ptr != nullptr) {
        if(!other_set.contains(ptr -> data)){
            return false; //return false if does not contain
        }
        ptr = ptr -> next;
    }
    return true; //return true if calling element is found in other_set
}
...
void Set<T>::clear() {
    ...
    while (head != nullptr) {
        next = head -> next;
        delete head; //delete while looping through each node
        head = next;
    }
}
