...
bool LinkedList<T>::contains(T myData) const {
    ...
    if (head == nullptr){
        ...
        while (ptr != nullptr){
            ...
            ptr = ptr -> next; // point head at next node
        }
        delete ptr; // delete the node ptr is pointing to
        return false;
    }
...
int LinkedList<T>::lastIndexOf(T myData) const {
    ...
    while(ptr!= nullptr){
        if(ptr -> data == myData){
            return index; //return index of element if found
        }
        ptr = ptr -> next; // point head at next node
        index++; //increment index each pass
    }
    return -1;
}
...
bool LinkedList<T>::remove(T myData) {
    ...
    if (head == nullptr ){
        ...
        while (ptr != nullptr && ptr -> data != myData) {
            previous = ptr;
            ptr = ptr -> next;
        }
        if (ptr) {
...
T LinkedList<T>::removeAt(int index) {
    ...
    returnValue = get(index); //return value to be deleted
    if (head == nullptr){
        return false;
    }
    Node<T> *ptr = head;// set ptr to what head points at
...
T LinkedList<T>::set(int index, T myData) {
    ...
    double returnValue = 0;
    returnValue = get(index); //return value to be deleted
    remove(returnValue); //remove value to be replaced
    add(index, myData); //add node with index and myData given
    return returnValue;
}
