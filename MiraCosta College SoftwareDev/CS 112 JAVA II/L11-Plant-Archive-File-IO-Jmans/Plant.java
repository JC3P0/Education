public class Plant {
    ...
    public Plant(String csv){
        ...
        String[] parts = csv.split(","); //should have three parts
        if (parts.length != 3){
            throw new IllegalArgumentException("CSV string does not have required number of values. CSV string = " + csv);
        }
        String name = parts[0];
        double temp;
        try{
            ...
            throw new IllegalArgumentException("CSV string doest have valid temp(doule) as second values = " + parts[1]);
        }
        String uses = parts[2];
